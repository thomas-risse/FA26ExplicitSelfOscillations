...
# Physical string parameters (linear harmonic)
rho = 7850
Radius = 1e-4
l0 = 0.65
T0 = 73
...
# Modal string parameters
Nmodes = 10


M = np.pi*Radius**2 * rho * l0 * np.ones(Nmodes) / 2
R = 2 * M * eta_0
K = T0 / l0 * np.pi**2 * np.arange(1, Nmodes + 1)**2 / 2
...
def FBow(t):
    return FBow0 * np.ones_like(t)


def vBow(t):
    return vBow0 * (t >= trise) + vBow0 * (t < trise) * t / trise
...
# Listening position
exList = 0.4
xProj = np.zeros(Nmodes)
for i in range(Nmodes):
    xProj[i] = np.sin(exList * np.pi * (i+1))
...
def run_simulation(sr: int, duration: float, fname: str) -> dict:
    ...
    with h5py.File(fname, "w") as f:
        f.attrs["DissipationMode"] = DissipationMode
        f.attrs["Nmodes"] = Nmodes
        f.attrs["ExPos"] = exPos
        f["M"] = M
        f["K"] = K
        f["R"] = R
    ...
    subprocess.run(
        ["../build/RunBowedStringModal", f"{path.realpath(fname)}"],
        check=True,
...
fname_ref = path.join(result_folder, "testBowedString_ref.hdf5")
ref = run_simulation(sr_ref, duration, fname_ref)
q_ref = ref["q"]                      # shape (N_ref, 3)
q_ref = q_ref[::int(sr_ref / sr0)]
p_ref = ref["p"]                      # shape (N_ref, 3)
p_ref = p_ref[::int(sr_ref / sr0)]

output_ref = xProj.T @ ref["p"][::int(sr_ref/sr0), :].T
if (idx_plot == Nsrs - 1):
    res_plot = ref
...
for i, sr in enumerate(sample_rates[:-1]):
    ...
    fname = path.join(result_folder, f"testBowedString_{sr}.hdf5")
    ...
    p_cur_window = res["p"][idxmin_cur:idxmax_cur]
    p_ref_window = p_ref[idxmin_ref:idxmax_ref]

    # Interpolate current solution onto the reference grid (within window)
    q_cur_resampled = q_cur_window[::int(sr/sr0)]
    p_cur_resampled = p_cur_window[::int(sr/sr0)]

    output = xProj.T @ res["p"][::int(sr/sr0), :].T
    diff = output - output_ref
    l2_errors.append(l2_norm(diff) / l2_norm(output_ref))

    write(path.join(result_folder, f"bowed{i}.wav"), sr0,
...
plt.tight_layout()
fig_conv.savefig(
    path.join(figure_folder, "Bowed_string_convergence.png"),
    bbox_inches="tight",
)
